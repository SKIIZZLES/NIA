import type { TranslationKeys } from './fr';

/**
 * Maghrebi Arabic / Darija (Morocco–Maghreb dialect), not MSA.
 * Arabic script; layout stays LTR — see README.
 */
const arMA: TranslationKeys = {
  common: {
    error: 'خطأ',
    or: 'ولا',
    email: 'الإيميل',
    password: 'كلمة السر',
    username: 'السميت',
    guest: 'ضيف',
    loading: 'كيتحمّل…',
    cancel: 'Cancel',
    back: 'Back',
  },
  brand: {
    tagline: 'فيديوات · ثقافات · مواهب · بلا حدود',
    heroPrimary: 'هنا، المواهب ديال أفريقيا كيمشيو بعيد',
    heroSecondary: 'أكثر من فيديوات — أفريقيا كاتحكي على راسها',
  },
  tabs: {
    home: 'الرئيسية',
    discover: 'اكتشف',
    create: 'نشر',
    notifications: 'الإشعارات',
    profile: 'البروفيل',
  },
  welcome: {
    createAccount: 'أنشئ كونط',
    signIn: 'دخل',
    mockHint:
      'Expo Go أصلي = مصادقة وهمية · Supabase كامل عبر الويب ولا البناء الأصلي',
    supabaseHint: 'مصادقة Supabase متّصلة',
    authMockBadge: 'AUTH MOCK MVP',
  },
  auth: {
    loginTitle: 'دخل',
    registerTitle: 'أنشئ كونط',
    signIn: 'دخل',
    createAccount: 'أنشئ الكونط ديالي',
    emailPlaceholder: 'you@email.com',
    usernamePlaceholder: 'السميت_ديالك',
    loginFail: 'ما قدرناش ندخلوك',
    registerFail: 'ما قدرناش نسجّلوك',
    mockLoginHint:
      'كيتقبل أي إيميل / كلمة سر. الجلسة محفوظة محلياً (AsyncStorage). حط EXPO_PUBLIC_SUPABASE_* فـ .env باش تفعّل Supabase.',
    supabaseLoginHint:
      'إيميل / كلمة سر عبر Supabase Auth. أنشئ كونط من شاشة التسجيل.',
    mockRegisterHint:
      'فورم تجريبي — بلا تحقق من السيرفر. كينشئ جلسة محلية.',
    supabaseRegisterHint:
      'تسجيل عبر Supabase Auth. البروفيل كيتخلق أوتوماتيك (محفّز SQL). عطّل «Confirm email» فـ Auth → Providers باش تجرّب بلا إيميل.',
    authMockBadge: 'AUTH MOCK MVP',
    authSupabaseBadge: 'AUTH SUPABASE',
  },
  google: {
    continue: 'كمّل مع Google',
    fail: 'الدخول بـ Google ما نجحش',
    alertTitle: 'Google',
    hintWeb: 'Google أصلي = بناء EAS ديال Android/iOS',
    hintMock: 'وضع وهمي: جلسة محلية (Expo Go ولا env ناقصة)',
    hintMissingId: 'حط EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID (EAS)',
  },
  snapchat: {
    continue: 'كمّل مع Snapchat',
    fail: 'الدخول بـ Snapchat ما نجحش',
    alertTitle: 'Snapchat',
    hintMock: 'وضع وهمي: جلسة محلية (Expo Go ولا env ناقصة)',
    hintMissingId: 'حط EXPO_PUBLIC_SNAP_CLIENT_ID (شوف SNAPCHAT_AUTH.md)',
    hintMissingFn: 'ضبط Snap Kit + نشر دالة snapchat-auth',
  },
  camera: {
    permissionTitle: 'الوصول للكاميرا',
    permissionBody: 'NIA محتاجة الكاميرا باش تصوّر الفيديو ديالك.',
    permissionDenied: 'الوصول للكاميرا ممنوع. حل إعدادات التيليفون باش تسمح.',
    allow: 'سمح بالكاميرا',
    openSettings: 'حل الإعدادات',
    micDenied: 'المايك ما مسموحش: الفيديو غادي يتصوّر بلا صوت.',
    allowMic: 'سمح بالمايك',
    record: 'بدا التصوير',
    stop: 'وقّف التصوير',
    close: 'سد الكاميرا',
    flip: 'بدّل الكاميرا',
    gallery: 'حل المعرض',
    recordFailed: 'التصوير ما نجحش. عاود جرب.',
    mountErrorTitle: 'الكاميرا ما متوفراش',
    mountErrorBody: 'الكاميرا ديال NIA ما قدرتش تبدا. غادي نحلو كاميرا التيليفون.',
  },
  create: {
    title: 'نشر',
    subtitleMock: 'وضع التجربة: النشر ديالك كيبقى فهاد الجهاز.',
    subtitleSupabase: 'النشر ديالك غادي يبان فالفيد ديال NIA.',
    pickMedia: 'ختار ميديا (المعرض)',
    noMedia: 'ما خترتيش ميديا',
    video: 'فيديو',
    image: 'صورة',
    captionLabel: 'التعليق والهاشتاغات',
    captionPlaceholder: 'وصف الموهبة ديالك… زيد #africa #culture',
    hashtagHint: 'الكلمات اللي بـ # فالتعليق كيوليو هاشتاغات منين تنشر.',
    categoryLabel: 'الفئة',
    limits: 'أقصى %{minutes} د · %{mb} مو',
    publish: 'انشر',
    publishedMockTitle: 'تنشر فوضع التجربة',
    publishedTitle: 'تّنشر',
    publishedMockBody: 'تزاد للخلاصة المحلية «لك».',
    publishedBody: 'النشر ديالك تنشر. لقاه فالفيد وفالبروفيل ديالك.',
    publishFail: 'النشر ما نجحش',
    alertTooLarge: 'الملف كبير بزاف',
    alertTooLong: 'الفيديو طويل بزاف',
    alertMediaRequired: 'الميديا ضروريّة',
    alertCategory: 'الفئة',
    errNoMedia: 'ختار فيديو ولا صورة باش تنشر.',
    errTooLarge: 'الملف فات الحجم الأقصى (%{mb} مو).',
    errTooLong: 'الفيديو فات المدة القصوى (%{minutes} د).',
    errCategoryRequired: 'ختار فئة للبوست ديالك.',
    film: 'صوّر',
    filmHint: 'صور دابا ولا ختار شي ملف من الݣاليري.',
    alertCamera: 'الكاميرا',
    errCameraDenied: 'سمح ليا بالكاميرا باش تصوّر فيديو.',
    coverLabel: 'غلاف (اختياري)',
    coverHint: 'الصورة اللي كتبان فالبروفيل / اكتشف. بلا غلاف، كتستعمل صورة NIA افتراضية.',
    pickCover: 'ختار غلاف',
    changeCover: 'بدّل الغلاف',
    clearCover: 'حيّد الغلاف',
    hubVideo: 'فيديو',
    hubPhoto: 'Photo',
    hubLive: 'Live',
    hubEvent: 'فعالية',
    pickVideo: 'ختار فيديو',
    pickPhoto: 'ختار صورة',
    takePhoto: 'ديّر تصويرة',
    filmHintVideo: 'معرض الفيديوات ولا الكاميرا. الغلاف اختياري للبروفيل.',
    filmHintPhoto: 'معرض الصور ولا الكاميرا.',
    errWrongMediaVideo: 'ختار فيديو لهاد الوضع.',
    errWrongMediaPhoto: 'ختار صورة لهاد الوضع.',
    addSound: 'زيد صوت',
    addSoundHint:
      'غير الأصوات المحمّلة / الأصلية — ما كاينش كاطالوݣ تجاري. ختار واحد من الأصوات ديالك ولا دخّل ملف صوتي.',
    pickSound: 'ختار صوت',
    stepOf: 'خطوة %{n} من %{total}',
    stepMediaTitle: 'الميديا ديالك',
    stepStyleTitle: 'الشكل',
    stepPublishTitle: 'النشر',
    continue: 'كمّل',
    otherFormats: 'صيغ أخرى',
    uploading: 'كيتصيفط…',
    uploadingCover: 'كيتصيفط الغلاف…',
    retry: 'عاود جرب',
    uploadCanceled: 'الإرسال تلغى.',
},
  profile: {
    defaultBio: 'بروفيل NIA',
    posts: 'المنشورات',
    followers: 'المتابعين',
    following: 'كنتبع',
    editProfile: 'عدّل البروفيل',
    viewPublic: 'شوف البروفيل العام ديالي',
    signOut: 'خرج',
    signIn: 'دخل',
    empty: 'ما كاين حتى منشور دابا.',
    tabPublications: 'Publications',
    tabArchives: 'Archives',
    tabSaves: 'Enregistrements',
    tabSeries: 'Séries',
    emptySeries: 'Aucune série pour l’instant.',
    emptyArchives: 'Aucune vidéo archivée.',
    emptySaves: 'Aucun enregistrement pour l’instant.',
    unarchive: 'Désarchiver',
    unsave: 'Retirer',
    unsaveTitle: 'Retirer des enregistrements ?',
    unsaveBody: 'Cette vidéo ne figuera plus dans vos enregistrements.',
    language: 'اللغة',
    appearance: 'Apparence',
  },
  notifications: {
    title: 'الإشعارات',
    subtitle: 'النشاط حول البروفيل والمنشورات ديالك.',
    emptyTitle: 'ما كاين والو دابا',
    emptyBody:
      'منين المجتمع يتفاعل مع المحتوى ديالك، غادي يبان هنا — بوضوح وبلا ضوضاء.',
    note:
      'دخل مع Supabase باش توصل الإعجابات والتعليقات والمتابعين الجداد. الرسائل الخاصة فالمرحلة 2.',
    someone: 'شي واحد',
    liked: 'عجبو الفيديو ديالك',
    commented: 'علّق على الفيديو ديالك',
    followed: 'تبعك',
    system: 'إشعار النظام',
    interacted: 'تفاعل معاك',
    justNow: 'دابا',
    minutesAgo: 'هادي %{count} د',
    hoursAgo: 'هادي %{count} س',
    daysAgo: 'هادي %{count} ي',
    filterAll: 'الكل',
    filterLikes: 'الإعجابات',
    filterComments: 'التعليقات',
    filterFollows: 'المتابعين',
    sectionToday: 'اليوم',
    sectionWeek: 'هاد السيمانة',
    sectionEarlier: 'قدم',
  },
  feed: {
    repost: 'عاود نشر',
    repostedBy: 'عاود نشر',
    repostSuccess: 'تمّت إعادة النشر',
    repostSuccessBody: 'الفيديو بان ف البروفيل و الفيد.',
    repostSuccessMock: 'إعادة النشر تزادت ف الفيد المحلي.',
    repostAlready: 'سبق ليك عاودتي نشر هاد الفيديو.',
    repostFail: 'ماقدرتش تعاود النشر',
    loginRequiredTitle: 'خاصك تدخل',
    loginRequired: 'دخل باش تعاود تنشر فيديو.',
    share: 'شارك',
    save: 'سجّل',
    saveLoginRequired: 'دخل باش تسجّل الفيديو.',
    rewind: 'رجّع 5 ثواني',
    forward: 'قدّم 5 ثواني',
    mute: 'طفّي الصوت',
    unmute: 'شعّل الصوت',
    pause: 'وقف',
    play: 'شغّل',
    comments: 'التعليقات',

    menuTitle: 'Options',
    report: 'Report',
    blockUser: 'Block user',
    archive: 'Archive',
    delete: 'Delete',
    archiveConfirmTitle: 'Archive this video?',
    archiveConfirmBody: 'It will leave the public feed but stay on your profile (Archived).',
    deleteConfirmTitle: 'Delete this video?',
    deleteConfirmBody: 'The video will be removed from the feed and your profile (soft delete).',
    archiveSuccess: 'Video archived',
    archiveSuccessBody: 'Removed from the public feed.',
    archiveSuccessMock: 'Archived (demo mode).',
    deleteSuccess: 'Video deleted',
    deleteSuccessBody: 'Removed from the feed and your profile.',
    deleteSuccessMock: 'Deleted (demo mode).',
    archivedBadge: 'Archived',
    videoNotFound: 'Video not found.',
  },
  sound: {
    title: 'الصوت',
    useThis: 'استعمل هاد الصوت',
    useCount: '%{count} استعمالات',
    originalLabel: 'صوت أصلي — %{handle}',
    originalOnly: 'غير الأصوات اللي حمّلها المجتمع. NIA ما كتقدّمش كاطالوݣ موسيقي تجاري.',
    notFound: 'ما لقيناش الصوت.',
    loadFail: 'ما قدرناش نحمّلو هاد الصوت.',
    loginRequired: 'دخل باش تزيد صوت.',
    uploadAudio: 'دخّل ملف صوتي',
    uploadTitleLabel: 'عنوان الصوت',
    uploadTitlePlaceholder: 'مثال بيت ديال الدار، صوت خارجي…',
    uploadFail: 'تحميل الصوت ما نجحش',
    mySounds: 'الأصوات ديالي',
    emptyOwn: 'ما كاين حتى صوت دابا. دخّل ملف صوتي (mp3, m4a, wav).',
    defaultTitle: 'صوت أصلي',
  },
  discover: {
    title: 'اكتشف',
    chipAll: 'الكل',
    emptyTitle: 'ماكاين والو دابا',
    emptyBodyCategory: 'ماكاين حتى فيديو فهاد العالم. كون الأول اللي ينشر.',
    emptyBodyAll: 'ماكاين حتى فيديو دابا. نشر الأول.',
    tileA11y: '%{handle}، %{count} إعجاب',
    categories: {
      afrique: 'أفريقيا',
      maghreb: 'المغرب العربي',
      diaspora: 'الشتات',
      culture: 'ثقافة',
      musique: 'موسيقى',
      mode: 'موضة',
      tech: 'تقنية وابتكار',
      food: 'مأكولات',
      sport: 'رياضة',
      actus: 'أخبار',
    },
  },
  events: {
    title: 'Events',
    subtitle: 'Concerts, meetups, culture — near you and beyond.',
    detailTitle: 'Event',
    createTitle: 'Create an event',
    createShort: 'Create',
    create: 'Create an event',
    openList: 'See events',
    discoverCta: 'Events',
    discoverCtaBody: 'Discover and create cultural events.',
    filterUpcoming: 'Upcoming',
    filterWeekend: 'This weekend',
    filterPopular: 'Popular',
    filterAll: 'All',
    catCulture: 'Culture',
    catMusique: 'Music',
    catSport: 'Sport',
    catFood: 'Food',
    catTech: 'Tech',
    catEducation: 'Education',
    catBusiness: 'Business',
    catOther: 'Other',
    empty: 'No events yet',
    emptyHint: 'Be the first to create one for your community.',
    mockHint: 'Connect Supabase and run migration 009 for events.',
    loadFail: 'Could not load events.',
    notFound: 'Event not found.',
    attend: 'Attend',
    attending: 'Going',
    attendFail: 'Could not update your attendance.',
    share: 'Share',
    attendees: '%{count} going',
    organizedBy: 'Hosted by %{handle}',
    fieldTitle: 'Title',
    fieldTitlePlaceholder: 'e.g. Afrobeat night in Dakar',
    fieldDescription: 'Description',
    fieldDescriptionPlaceholder: 'Vibe, lineup, what to expect…',
    fieldCover: 'Cover image',
    coverHint: 'Optional — a strong image draws more people.',
    pickCover: 'Choose an image',
    changeCover: 'Change image',
    fieldStartsDate: 'Start date & time',
    fieldEndsDate: 'End (optional)',
    dateHint: 'Date YYYY-MM-DD · time HH:MM',
    fieldLocation: 'Venue',
    fieldLocationPlaceholder: 'Address or place name',
    fieldCity: 'City',
    fieldCityPlaceholder: 'e.g. Abidjan',
    fieldCountry: 'Country',
    fieldCountryPlaceholder: 'e.g. Côte d’Ivoire',
    fieldCategory: 'Category',
    publish: 'Publish event',
    publishSuccess: 'Event published',
    publishFail: 'Could not publish',
    titleRequired: 'Give your event a title.',
    categoryRequired: 'Choose a category.',
    dateRequired: 'Enter a valid start date and time.',
    endDateInvalid: 'Invalid end date / time.',
    loginRequiredTitle: 'Sign in required',
    loginRequired: 'Sign in to attend an event.',
    loginRequiredCreate: 'Sign in to create an event.',
  },
  live: {
    title: 'Live',
    subtitle: 'Schedule and follow cultural lives — real-time streaming is coming soon.',
    detailTitle: 'Live',
    createTitle: 'Schedule a live',
    createShort: 'Create',
    create: 'Start a live',
    startCta: 'Start a live',
    openList: 'See lives',
    discoverCta: 'Live',
    discoverCtaBody: 'Scheduled lives and streaming prep.',
    sectionLive: 'Live now',
    sectionScheduled: 'Scheduled',
    sectionPopular: 'Popular',
    soonBanner: 'Coming soon — live infrastructure to wire up',
    soonBannerBody:
      'NIA is preparing lives (metadata, scheduling, UI). No real video stream yet — Mux / LiveKit not connected.',
    createNotice:
      'This saves a scheduled live (metadata only). No WebRTC / Mux broadcast yet — the real player comes later.',
    placeholderBody:
      'The streaming provider is not connected yet. This page shows live info — not a fake video player.',
    emptyLive: 'Nobody is live right now. Honest: no fake stream.',
    emptyScheduled: 'No scheduled lives yet.',
    emptyPopular: 'No popular lives yet.',
    mockHint: 'Connect Supabase and run migration 010 for lives.',
    loadFail: 'Could not load lives.',
    notFound: 'Live not found.',
    statusLive: 'LIVE',
    statusScheduled: 'Scheduled',
    statusEnded: 'Ended',
    statusCancelled: 'Cancelled',
    viewers: '%{count} viewers',
    hostedBy: 'Hosted by %{handle}',
    scheduledLabel: 'Scheduled',
    fieldTitle: 'Title',
    fieldTitlePlaceholder: 'e.g. Afrobeat live session',
    fieldDescription: 'Description',
    fieldDescriptionPlaceholder: 'What will you talk about?',
    fieldThumbnail: 'Thumbnail (optional)',
    thumbnailHint: 'Optional — promo image for the live.',
    pickThumbnail: 'Choose an image',
    changeThumbnail: 'Change image',
    fieldScheduled: 'Scheduled date & time',
    dateHint: 'Date YYYY-MM-DD · time HH:MM',
    fieldVisibility: 'Visibility',
    visPublic: 'Public',
    visFollowers: 'Followers',
    visPrivate: 'Private',
    fieldCategory: 'Category',
    catCulture: 'Culture',
    catMusique: 'Music',
    catSport: 'Sport',
    catFood: 'Food',
    catTech: 'Tech',
    catEducation: 'Education',
    catBusiness: 'Business',
    catOther: 'Other',
    publish: 'Schedule live',
    publishSuccess: 'Live scheduled',
    publishSuccessBody: 'Saved. Real broadcasting arrives when live infra is wired.',
    publishFail: 'Could not schedule the live',
    titleRequired: 'Give your live a title.',
    categoryRequired: 'Choose a category.',
    dateRequired: 'Enter a valid date and time.',
    endStream: 'End live',
    endFail: 'Could not end the live.',
    loginRequiredTitle: 'Sign in required',
    loginRequiredCreate: 'Sign in to schedule a live.',
  },
  search: {
    title: 'Search',
    placeholder: 'People, videos, sounds, #hashtags, events…',
    hint: 'Search creators, posts, sounds, events, or hashtags.',
    tabAll: 'All',
    tabPeople: 'People',
    tabPublications: 'Posts',
    tabSounds: 'Sounds',
    tabEvents: 'Events',
    tabHashtags: 'Hashtags',
    sectionPeople: 'People',
    sectionPublications: 'Posts',
    sectionSounds: 'Sounds',
    sectionEvents: 'Events',
    sectionHashtags: 'Hashtags',
    emptyTitle: 'No results',
    emptyBody: 'Try another keyword. Simple text match — no ranking.',
    loadFail: 'Could not run search.',
    noCaption: 'No caption',
    hashtagHint: 'See posts',
    openA11y: 'Open search',
  },
  series: {
    detailTitle: 'سلسلة',
    createTitle: 'سلسلة جديدة',
    createCta: 'أنشئ سلسلة',
    addTitle: 'زيد لسلسلة',
    addHint: 'ختار سلسلة ولا أنشئ وحدة جديدة.',
    addToSeries: 'زيد لسلسلة',
    fieldTitle: 'العنوان',
    fieldTitlePlaceholder: 'مثال حكايات الليل',
    fieldDescription: 'Description',
    fieldDescriptionPlaceholder: 'على شنو كتهضر هاد السلسلة؟',
    fieldCover: 'الغلاف',
    coverHint: 'اختياري — صورة إعلان السلسلة.',
    pickCover: 'ختار صورة',
    changeCover: 'بدّل الصورة',
    publish: 'أنشئ السلسلة',
    publishSuccess: 'تنشأت السلسلة',
    publishFail: 'ما قدرناش ننشئو السلسلة',
    titleRequired: 'عطي عنوان للسلسلة.',
    loginRequiredTitle: 'خاصك تدخل',
    loginRequiredCreate: 'دخل باش تنشئ سلسلة.',
    mockHint: 'وصل Supabase وطبّق migration 011 للسلاسل.',
    notFound: 'ما لقيناش السلسلة.',
    loadFail: 'ما قدرناش نحمّلو السلسلة.',
    emptyEpisodes: 'ما كاينة حتى حلقة دابا. زيد فيديو من قائمة «…».',
    emptyOwn: 'مازال ما عندكش سلسلة.',
    episodeLabel: 'حلقة %{n}',
    episodeCount: 'الحلقات: %{count}',
    removeEpisodeTitle: 'تحيّد هاد الحلقة؟',
    removeEpisodeBody: 'الفيديو غادي يبقى فالبروفيل ديالك، ولكن خارج هاد السلسلة.',
    removeEpisode: 'حيّد',
    deleteTitle: 'تحيّد السلسلة؟',
    deleteBody: 'الفيديوات ما غاديش يتحيّدو — غير السلسلة اللي غادي تمشي.',
    delete: 'حيّد السلسلة',
    addedTitle: 'تزاد للسلسلة',
    addedBody: '«%{title}» دابا فيها هاد الفيديو.',
    missingVideo: 'الفيديو ناقص.',
  },
  filter: {
    label: 'NIA Filters',
    hint: 'Color overlay preview after capture. No face AR or native LUT (V3).',
    none: 'None',
    feedBadge: 'NIA Filter',
    mvpNote: 'Preview filters (overlay / approx. matrix). Real LUT / AR = V3.',
    catBeaute: 'Beauty',
    catLumiere: 'Light',
    catPortrait: 'Portrait',
    catCulture: 'Culture',
    catAfrique: 'Africa',
    catDiaspora: 'Diaspora',
    catFun: 'Fun',
    catNia: 'NIA Originals',
  },
  appearance: {
    title: 'Apparence',
    subtitle: 'Personnalisez les couleurs de l’interface pour ce compte. Cela ne change pas le contenu des publications.',
    original: 'NIA Original',
    sable: 'Sable',
    terre: 'Terre',
    bronze: 'Bronze',
    nuit: 'Nuit',
    clair: 'Clair',
    auto: 'Automatique',
    autoHint: 'Suit le thème clair / sombre du système',
  },
  errors: {
    boundaryTitle: 'وقع شي خطأ',
  },
  language: {
    label: 'اللغة',
    choose: 'اختار اللغة',
    close: 'سدّ',
    fr: 'Français',
    en: 'English',
    es: 'Español',
    pt: 'Português',
    sw: 'Kiswahili',
    ha: 'Haoussa',
    'ar-MA': 'Darija',
    'ar-SD': 'Soudanais',
    yo: 'Yorùbá',
    zu: 'isiZulu',
    am: 'Amharique',
    wo: 'Wolof',
    ln: 'Lingála',
    'ar-EG': 'Égyptien',
    ig: 'Igbo',
    ff: 'Pulaar',
    bm: 'Bambara',
    ak: 'Akan',
    mnk: 'Mandingue',
    dyo: 'Diola',
  },
};

export default arMA;
